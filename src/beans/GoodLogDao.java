...
public class GoodLogDao {
	...
	public int addGood(Map param) {
		SqlSession sql = factory.openSession(); // JDBC의 Connect과정
		try {
			int r = sql.insert("goodlog.addDataUsingMap", param);
			if(r==1) 
				sql.commit();
			return r;
		} catch(Exception e) {
	...
	public int incGoodByNo() {
		SqlSession sql = factory.openSession(); // JDBC의 Connect과정
		try {
			int r = sql.insert("goodlog.incGoodByNo");
			if(r==1) 
				sql.commit();
			return r;
		} catch(Exception e) {
	...
	public List<Integer> getSomeDataByActor(String id) {
